...
is also centralized: Python tooling (e.g., `black`, `isort`, `pylsp`) and Rust
helpers (e.g., `rust-analyzer`, `cargo`, `clippy`) are pre-wired in the global
`after/` directory so they work for every project mounted into the container.
...
Vundle. Highlights include YouCompleteMe, ALE, UltiSnips, NERDTree, FZF/FZF.vim,
Vimspector, vim-dispatch, vim-arduino, vista.vim, vim-clap, and git integrations,
