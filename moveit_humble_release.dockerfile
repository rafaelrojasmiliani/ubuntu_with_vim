...
# Separte, one layeer for install basics, other for copy, other for install plugins
...
RUN chmod 777 /etc/vim &&  mkdir -p /etc/vim/bundle && chmod 777 /etc/vim/bundle && \
    ...
    git clone https://github.com/ilyachur/cmake4vim /etc/vim/bundle/cmake4vim && \
